...
use DB;
use Excel;
use App\Exports\TahunjurFTExport;
use App\Exports\JaluruktFTExport;

class FTController extends Controller
{
  public function index()
  {
    $jurusan=FTController::jurusan();
    $jalur=FTController::jalur();

    return view('report.ft.report',compact('jalur','jurusan'));
  }
  ...
  public function tahunjur()
  {
    ...
    if(isset($_POST['year'],$_POST['semester'],$_POST['jurusan']))
    {
      ...
      $query = DB::connection('07')->select("SELECT count(mhsNiu) AS jumlah FROM mahasiswa WHERE mhsProdiKode=$prodi
      ...
      return view('report.ft.report', compact('query','prodi','jurusan','jalur'));
  }
  ...
  public function jalurukt()
  {
    ...
    if(isset($_POST['jlrmsk'])){
        ...
        $query0 = DB::connection('07')->select("SELECT count(mhsNiu) AS jumlah
        ...
        $query1 = DB::connection('07')->select("SELECT mhsNiu,mhsNama,s_jalur_ref.jllrNama as jalur,program_studi.prodiNamaResmi as prodi,
    ...
    return view('report.ft.report',compact('jalur','jurusan','query0','query1'));
  }

  public static function jurusan()
  {
    return DB::connection('07')
    ->table('program_studi')
    ->select('prodiKode','prodiNamaResmi')
    ->where('prodiFakKode','=','07')
  ...
  {
    return Excel::download(new TahunjurFTExport, 'tahunjur.xlsx');
  }

  public function juexport()
  {
    return Excel::download(new JaluruktFTExport, 'jalurukt.xlsx');
  }
}
